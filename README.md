...
* Service APIs to enable:
  ...
  * Cloud Build API
  ...
  Or just run the following from the Cloud Shell:
...
2. Install Docker. Just follow the [official documentation][13], but these are the instructions I followed:
   ...
   You might as well add yourself to the `docker` group so you don't have to use `sudo` for every docker command:
...
4. Optional. Create a [firewall rule][14] tag to attach to the GCE instance. This rule will allow incoming traffic to the port 8000, which is what we will be using. This is not necessary as it will just allow us to access the application remotely.
...
1. Create the Artifact Registry repository.
       ...
       gcloud artifacts repositories create fastapi-repository --repository-format=docker --location=europe-west2

2. [Allow API access][16] from the GCE instance to the Storage API. Unfortunately it's required to turn off the GCE instance for this purpose, which is a bit cumbersome: 

       gcloud compute instances stop <GCE_INSTANCE>
       gcloud compute instances set-service-account <GCE_INSTANCE> --scopes=cloud-platform
       gcloud compute instances start <GCE_INSTANCE>

3. Grant IAM permission at repository level.

       gcloud artifacts repositories add-iam-policy-binding fastapi-repository \
           --location europe-west2 \
           --member=serviceAccount:<PROJECT_NUM>-compute@developer.gserviceaccount.com \
           --role=roles/artifactregistry.writer

From the GCE instance, configure [docker authentication][17]: 

       gcloud auth configure-docker europe-west2-docker.pkg.dev

### Deployment to Cloud Run from Artifact Registry
...
We will use [Artifact Registry][7] (AR) as it's the evolution of Container Registry. To [push to AR][8] it's needed to tag the image with a particular format and push it to the created AR repository.
    ...
    docker build -t europe-west2-docker.pkg.dev/<PROJECT_ID>/fastapi-repository/fastapi .
...
Note that you must provide your own project ID and must be on the same project as the target Cloud Run service. Alternatively we could've tagged the image that we already have instead of building a new one (the end result is equivalent):
    ...
    docker tag fastapi europe-west2-docker.pkg.dev/<PROJECT_ID>/fastapi-repository/fastapi
    
Once the image is tagged we can push it to AR with the following instruction:

    docker push europe-west2-docker.pkg.dev/<PROJECT_ID>/fastapi-repository/fastapi
...
At this point, the image is accessible from the created Artifact Registry repository on our GCP project, which means that it can be pulled by anyone with enough permissions. This time, we are going to use it to deploy to Cloud Run:
    ...
    gcloud run deploy fastapi \
        --image europe-west2-docker.pkg.dev/<PROJECT_ID>/fastapi-repository/fastapi \
        --platform managed \
        --region europe-west2 \
...
To allow deployments to Cloud Run from Cloud Build, [extra permissions][15] are needed to Cloud Build's Service Account:
...
Cloud Build will work by uploading the working directory to Cloud Storage and it will follow a set of instructions that we provide. Those instructions are listed in the [`cloudbuild.yaml`][9] file. Please give the file a read to understand in detail what it does. To sum up, it's just the three instructions (`docker build`, `docker push` and `gcloud run deploy`) so Cloud Build knows how to build and deploy our application.
...
[15]: https://cloud.google.com/build/docs/quickstart-deploy#grant_permissions
[16]: https://cloud.google.com/artifact-registry/docs/access-control#compute
[17]: https://cloud.google.com/artifact-registry/docs/docker/authentication#gcloud-helper
